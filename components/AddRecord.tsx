...
const FB_EXPENSE = ['Продукты', 'Транспорт', 'Кафе и рестораны', 'Связь', 'Развлечения', 'Подписки', 'Здоровье', 'Прочее'];
const FB_INCOME  = ['Зарплата', 'Фриланс', 'Подработка', 'Прочее'];
...
function isIncomeType(t: string) { return t.includes('Доход') || t.includes('накопления'); }
...
export default function AddRecord() {
  ...
  const [expenseCats, setExpenseCats] = useState<string[]>(FB_EXPENSE);
  const [incomeCats,  setIncomeCats]  = useState<string[]>(FB_INCOME);
  const [catsReady,   setCatsReady]   = useState(false);

  const [manageTab, setManageTab]   = useState<'expense' | 'income'>('expense');
  ...
  const [opCat,  setOpCat]  = useState(FB_EXPENSE[0]);
  ...
  // Список категорий зависит от типа операции
  const activeCats = isIncomeType(opType) ? incomeCats : expenseCats;

  // Загружаем категории при монтировании
  useEffect(() => {
    fetch('/api/categories').then(r => r.json()).then(d => {
      if (d.expense?.length) setExpenseCats(d.expense);
      if (d.income?.length)  setIncomeCats(d.income);
      // Устанавливаем дефолтную категорию по текущему типу
      setOpCat((d.expense?.[0]) ?? FB_EXPENSE[0]);
      setCatsReady(true);
    }).catch(() => setCatsReady(true));
  }, []);

  // При смене типа — сбрасываем категорию на первую в нужном списке
  useEffect(() => {
    const list = isIncomeType(opType) ? incomeCats : expenseCats;
    setOpCat(list[0] ?? '');
  }, [opType]); // eslint-disable-line react-hooks/exhaustive-deps

  function applyCatsResponse(d: { expense?: string[]; income?: string[] }) {
    if (d.expense) setExpenseCats(d.expense);
    if (d.income)  setIncomeCats(d.income);
  }
  ...
  async function addCategory() {
    ...
    try {
      ...
      else { applyCatsResponse(d); showToast(`Категория «${name}» добавлена`, 'success'); }
      setNewCat('');
  ...
  async function deleteCategory(name: string) {
    try {
      ...
      if (d.ok) {
        applyCatsResponse(d);
        if (opCat === name) {
          const list = isIncomeType(opType) ? (d.income ?? incomeCats) : (d.expense ?? expenseCats);
          setOpCat(list[0] ?? '');
        }
  ...
  const currentList = manageTab === 'income' ? incomeCats : expenseCats;
  ...
  return (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={card}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: showManage ? 16 : 8 }}>
        ...
        <div style={{ display: 'flex', gap: 0, marginBottom: 16, background: C.surface2, borderRadius: 10, padding: 4 }}>
          {(['expense', 'income'] as const).map(tab => (
            <button key={tab} onClick={() => { setManageTab(tab); setNewCat(''); }}
              style={{
                flex: 1, padding: '7px 0', borderRadius: 7, border: 'none', fontSize: 13, fontWeight: 600,
                cursor: 'pointer', transition: '.2s',
                background: manageTab === tab ? C.surface : 'transparent',
                color: manageTab === tab ? C.text : C.sub,
              }}>
        ...
        {/* Чипы */}
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: showManage ? 16 : 0 }}>
          {currentList.map(cat => (
            <div key={cat} style={{
              display: 'flex', alignItems: 'center', gap: showManage ? 6 : 0,
              background: C.surface2, border: `1px solid ${C.border}`,
              borderRadius: 20, padding: showManage ? '5px 10px 5px 14px' : '5px 12px',
              fontSize: 13, color: C.text,
            }}>
              <span>{cat}</span>
              {showManage && (
                <button onClick={() => deleteCategory(cat)}
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.sub, fontSize: 15, lineHeight: 1, padding: '0 2px' }}>
                  ×
                </button>
              )}
            </div>
          ...
          {currentList.length === 0 && (
            <span style={{ color: C.sub, fontSize: 13 }}>Нет категорий</span>
          )}
        </div>
        ...
        {showManage && (
          <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
            <input ref={newCatRef} type="text" value={newCat}
              ...
              placeholder={`Новая категория ${manageTab === 'income' ? 'дохода' : 'расхода'}…`}
              style={{ ...inputS, flex: 1 }}
            />
